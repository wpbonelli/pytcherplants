...
if [[ "$ilastik" -eq 1 ]]; then
  ...
  echo "$segmented"
  python3 /opt/pytcherplants/pytcher_plants/cli.py ilastik postpc -i "$input" -m "$segmented" -o "$output"
  input="$output/$(basename "$input" | cut -d. -f1)_masked.tiff"
fi

# geometric trait and color analysis from masked plant tissues
python3 /opt/pytcherplants/pytcher_plants/cli.py colors analyze -i "$input" -o "$output"
