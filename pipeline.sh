...
if [[ "$ilastik" -eq 1 ]]; then
  ...
  /opt/ilastik/ilastik-1.4.0b21-gpu-Linux/run_ilastik.sh --headless --project="/opt/pytcher-plants/pytcherplants.ilp" --output_format="tiff" --output_filename_format="$output/{nickname}_segmented.tiff" --export_source="Simple Segmentation" "$input"

  # postprocess ilastik pixel segmentation results
  segmented="$output/$(basename "$input" | cut -d. -f1)_segmented.tiff"
  echo "$segmented"
  python3 /opt/pytcher-plants/pytcher_plants/cli.py ilastik postpc -i "$input" -m "$segmented" -o "$output"
  input="$output/$(basename "$input" | cut -d. -f1)_masked.tiff"
fi

# geometric trait and color analysis from masked plant tissues
python3 /opt/pytcher-plants/pytcher_plants/cli.py traits analyze -i "$input" -o "$output"
